...
async function sync() {
    ...
    try {
        ...
        const records = await youfibreDb.query(`SELECT db_records.id FROM db_records left join schemas on (schemas.id = db_records.schema_id) where schemas.entity_name = 'Project'`);
        ...
        for(const record of records) {
            ...
            if(totalCost[0]) {
                ...
                update.entity = `ProjectModule:Project`;
                update.properties = {
