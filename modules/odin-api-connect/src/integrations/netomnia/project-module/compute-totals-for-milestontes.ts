...
async function sync() {
    ...
    try {
        ...
        const records = await youfibreDb.query(`SELECT db_records.id FROM db_records left join schemas on (schemas.id = db_records.schema_id) where schemas.entity_name = 'Milestone'`);
        ...
        for(const record of records) {
            ...
            WHERE Record__Schema.entity_name = 'Task'
                AND EXISTS (
                     ...
                     WHERE parent_schema.entity_name = 'Milestone'
            ...
            if(totalCost[0]) {
                ...
                update.entity = `ProjectModule:Milestone`;
                update.properties = {
