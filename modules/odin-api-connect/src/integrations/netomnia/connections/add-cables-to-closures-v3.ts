...
async function execute() {
    ...
    try {
        ...
        const chunkedConnections = chunkArray(connectionMappings.reverse(), 100);
 ...
 * @param schemaType
 */
export const getAllRecordsByEntity = async (
    odinDb: any,
    moduleName: string,
    entityName: string,
    schemaType: string,
 ...
 * @param recordIds
 */
export const getManyRecordsDetail = async (recordIds: string) => {
...
async function createCableConnection(
    ...
    newRecord.properties = {
        ...
        ClosureExternalRef: getProperty(closure, 'ExternalRef'),
        ClosureType: getProperty(closure, 'ClosureType'),
        PortId: port.id,
