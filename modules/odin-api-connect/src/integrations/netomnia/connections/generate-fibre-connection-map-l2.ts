...
async function execute() {
    ...
    try {
        ...
        const closuresNoConnections = await getAllClosuresWithNoFibreConnections(l2ClosureIds)
...
async function createFibreConnectionMappings(closureId: string) {
    ...
    const availableFibers = allFibres.filter(elem => elem['fiber_state'] === null).filter(elem => !searchedStartingFibres.includes(
        elem['fiber_id']))
    const firstAvailableFiber = availableFibers[0]

    console.log('_ALL_FIBRES', allFibres)
    console.log('_AVAILABLE_FIBRES', availableFibers)

    console.log('----FROM_CLOSURE:   ', inClosure['from_closure_ext_ref'])
    console.log('----IN_CABLE:       ', inClosure['in_cable_ext_ref_from_closure'])
    console.log('----SEARCH_NEXT_CABLE: ', searchNextTubeFibre)
    ...
    console.log('_OUT_FIBRES_TO_CONNECT', outFibresToConnect)
    ...
    console.log('------NEW_CONNECTIONS_NEEDED', newConnectionsNeeded)
    ...
    for(let i = 0; i < newConnectionsNeeded; i++) {
        ...
        const nextFibre = availableFibers[i]
        searchedStartingFibres.push(nextFibre['fiber_id'])
        ...
        await goToNextClosure(inCableId, 'L1', inClosureType, 'IN', closureId, nextFibre)

    }

    console.log('_RAW_FIBRE_TRACES', fibreTraces)
    ...
    if(allFibres[0]) {
        ...
        for(const inFibre of inFibresToConnect) {
            ...
            const fibreConnections = await getFibreConnectionsByFibreId(inFibre['fibreOutId'])
            ...
            console.log('fibreConnection', fibreConnection);
            if(fibreConnection) {
...
async function getNextAvailableFibreUpstreamCable(
    ...
    if(firstInCable) {
        ...
        if(firstFibre) {
            ...
            const cableFibresDownstream = await getAllFibresByCableId(cableId)
            // console.log('cableFibresDownstream', cableFibresDownstream)
            if(isLoop) {

                matchingFibreOut = await getAvailableFibre(cableId, searchNextTubeFibre)
        ...
        const matchingFibreIn = await getAvailableFibre(firstInCable['cable_id'], searchNextTubeFibre)
        ...
        if(!matchingFibreIn) {
            ...
            const nextFibre = await getAvailableFibre(cableId, searchNextTubeFibre, true)

            if(nextFibre) {

                const tubeFibreKeyIn = constructTubeFibreKey(nextFibre['tube_number'], nextFibre['fiber_number'])

                searchedFibreKeys.push(tubeFibreKeyIn)

                searchNextTubeFibreOverride = tubeFibreKeyIn
        ...
        } else if(matchingFibreIn) {

            // get the upstream fibre connections
            const fibreConnections = await getFibreConnectionsByFibreId(matchingFibreIn['fiber_id'])

            console.log('_FIBRE_CONNECTION_UPSTREAM_ALL', fibreConnections)

            // we need to find the connections for the in cable
            // loop cables have multiple connections
            const fibreConnection = fibreConnections.find(elem => elem[`cable_${direction}`.toLowerCase()] === firstInCable['cable_id'])
            ...
            searchedFibreKeys.push(tubeFibreKeyOut)
...
async function getAvailableFibre(cableId: string, tubeFibre: string, filterOutSearched?: boolean) {
    ...
    if(searchedFibreKeys && searchedFibreKeys.length > 0 && filterOutSearched) {

        matchingFibre = cableFibres.find(fibre => !searchedFibreKeys.includes(constructTubeFibreKey(
            fibre['tube_number'],
            fibre['fiber_number'],
 ...
 *
 */
async function filterOutFibresAlreadyConnected(inCableId: string, outFibres: any) {

    // get the cable fibres
    const cableFibres = await getAllFibresByCableId(inCableId)

    // find the matching fibre by tube and fibre
    const matchingFibre = cableFibres.find(fibre => constructTubeFibreKey(
        fibre['tube_number'],
        fibre['fiber_number'],
    ) && fibre['fiber_state'] !== null);
    ...
    if(matchingFibre) {
        // get the upstream fibre connections
        const fibreConnections = await getFibreConnectionsByFibreId(matchingFibre['fiber_id'])

        // we need to check if the total number of fiber connections is less than the
        const fiberConnectionsByCable = fibreConnections.filter(elem => elem['cable_in'] === inCableId)

        if(fiberConnectionsByCable && fiberConnectionsByCable.length > 0) {

            const connectedFibreIds = fiberConnectionsByCable.map(elem => elem['fiber_out_id']);

            // filter out the already connected out fibres
            return outFibres.filter(elem => !connectedFibreIds.includes(elem['fibre_id']));
...
async function createFibreConnections(
    ...
    console.log('_CONNECTIONS_TO_CREATE', inFibresToConnect)
    console.log('_CONNECTIONS_TO_DELETE', connectionsToDelete)

    // create new connections with splitters
    for(const inConnection of inFibresToConnect) {
        ...
        const connectionDetails = {
            ...
            inClosureExt: inConnection['outClosureExt'],  // we trace up so the OUT params from the inConnection are
            cableInId: inConnection['cableOutId'],
            tubeFibreIn: inConnection['tubeFibreKeyOut'],
            tubeInId: inConnection['tubeOutId'],
            fibreInId: inConnection['fibreOutId'],
        ...
        await createSplitterConnections(connectionDetails, 4);

    }


    console.log('_CONNECTIONS', connections)
    console.log('_CONNECTIONS_LENGTH', connections.length)

    console.log('_CONNECTIONS_TO_CREATE', inFibresToConnect.length)
    ...
    for(const item of connectionsToDelete) {
        ...
        if(item['connection_id']) {
            ...
            console.log('connection_id', item['connection_id'])

            if(!dryRun) {
                const deleteRes = await deleteRecord('FeatureConnection', item['connection_id'])
...
async function createSplitterConnections(connectionDetails, splitterCount) {

    for(let i = 0; i < Number(splitterCount); i++) {

        await parseFibreConnection(connectionDetails, inFibresToConnect.length);

    }

    console.log('RETURNED_HERE')
 ...
 */
async function getNextSplitter(trays, connectionsPerFibre) {
 ...
 * @param slotNumber
 * @param startingSlotNumber
 */
async function getNextAvailableSlot(connectionDetails, slotNumber: number, startingSlotNumber: number) {
    ...
    for(const slot of slots) {
        if(slot['trays']) {
            for(const slotTray of slot['trays']) {

                const tray = await getSplittersByTrayId(slotTray['id']);
                ...
                if(!tray['splitters']) {
                    ...
                    const splittersFiltered = tray['splitters'].filter(elem => usedSplitterIds.filter(
                        num => num === elem['splitter_number']).length < 4)

                    // const availableSplitterCount = tray['splitters'].filter(splitter => !splitter['connections']);
                    // console.log('availableSplitterCount', availableSplitterCount)

                    console.log('splittersFiltered', splittersFiltered);

                    if(splittersFiltered.length > 0) {

                        console.log('HAS_AVAILABLE_SLOT')
    ...
    console.log('AVAIL_SLOT_NUMBER', availableSlots[0]['slot_number'])
    console.log('Number(slotNumber)', Number(slotNumber))

    if(Number(availableSlots[0]['slot_number']) > Number(slotNumber || 0)) {
        console.log('SLOT_NUMBER_IS_LESS')
    ...
    return { slot, startingSlotNumber };
}
...
async function parseFibreConnection(connectionDetails: any, totalFibres: number, slotNumber?: number) {
    ...
    const __ret = await getNextAvailableSlot(connectionDetails, slotNumber, startingSlotNumber);
    const slot = __ret.slot;
    startingSlotNumber = __ret.startingSlotNumber;
    ...
    if(slot) {
        ...
        console.log('totalConnectionsForFibre', totalConnectionsForFibre)
        console.log('connectionsPerFibre', connectionsPerFibre)

        if(connections.length >= totalFibresAfterSplitter) {

            console.log('BREAK_THE_LOOP')
            ...
            if(trays) {
                let { nextSplitter, tray } = await getNextSplitter(trays, connectionsPerFibre);

                // if there is no nextSplitter go to the next slot

                if(!nextSplitter) {

                    console.log('NO_SPLITTER_SEARCH_AGAIN', nextSplitter)
                ...
                } else if(connections.length < totalFibresAfterSplitter) {
 ...
 *
 ...
 5    "L4"
 */
async function getAllClosuresWithNoFibreConnections(l2ClosureIds: string[]) {
    ...
    return await odinDb.query(`
        ...
        and c1.value IN ('3')
        ${l2ClosureIds ? `and c.value IN (${l2ClosureIds.map(elem => `'${elem}'`)})` : 'and c.value IS NOT NULL'}
...
const getCableFibreConnectionsByCableId = async (cableIds: string[]) => {
    ...
    return await odinDb.query(`
        ...
        left join db_records_columns c11 on (c11.record_id = cr.id and c11.column_name = 'FibreOutId')
        left join db_records_columns c12 on (c12.record_id = cr.id and c12.column_name = 'TubeFibreOut')
