...
async function sync() {
    try {
        ...
        const records = await pg.query(
            ...
            AND pipelines_stages.key = 'OrderStageCancelled'`);
        ...
        for(const record of records) {
            ...
            const activeStageId = '94ec5936-7ee0-418b-a450-0a7ac619357d';
            ...
            const cancelledEvent = userEvents.find(elem => elem.revision['stageId'] === order.stage.id);
            const activeEvent = userEvents.find(elem => elem.revision['stageId'] === activeStageId);
            ...
            updateDto.properties = {
                CancelledDate: cancelledEvent ? moment(cancelledEvent.created_at).format('YYYY-MM-DD') : order.stageUpdatedAt,
                ActiveDate: activeEvent ? moment(activeEvent.created_at).format('YYYY-MM-DD') : undefined,
            };
