...
async function sync() {
    ...
    try {
        ...
        AND EXISTS (
            ...
            WHERE EXISTS (
                ...
                WHERE records_columns.record_id = associations.child_record_id
                    AND records_columns.value IN ('cancelled', 'failed')
                    AND records_columns.value NOT IN ('active', 'submitted', 'created')
                    )
        ...
        for(const item of data) {
            ...
            newEmail.templateId = 'd-1908c1c4bc4a4896aa6e58e795ee21e2';
