...
class WorkInstallOrderStageButtons extends React.Component<Props, State> {
  ...
  render() {
    const { record, pipelineReducer } = this.props;

    const inProgress = this.state.pipeline?.stages?.find((elem: PipelineStageEntity) => elem.key === 'WorkOrderStageInProgress');
    const done = this.state.pipeline?.stages?.find((elem: PipelineStageEntity) => elem.key === 'WorkOrderStageDone');
      ...
      <>
        ...
        <Row gutter={18} style={{ marginBottom: 16, marginTop: 16 }}>
          <Col span={8}>
            <Button
              type="primary"
              disabled={record?.stage?.isSuccess || record?.stage?.isFail || record?.stage?.key ===
...
export default connect(mapState, mapDispatch)(WorkInstallOrderStageButtons);
