...
class StepView extends React.Component<Props, State> {
  ...
  setInitailStepData = (n: number) => {
    ...
    for(let i = 1; i <= n; i++) {
      if(i === 1) {
        tempArr.push({ isNextDisabled: true });
      } else {
        tempArr.push({ isNextDisabled: false });
      }
      if(i === n) {
  ...
  prev = () => {
    const { stepViewReducer, changeStep } = this.props;
      ...
      <>
        ...
        <div className="steps-action">
          {stepViewReducer.currentStep > 0 && (
            <Button style={{ margin: '0 8px' }} onClick={() => this.prev()} disabled={previousDisabled}>
              Previous
            </Button>
          )}
