...
@Injectable()
export class DbRecordsColumnsServiceHelpers {
  ...
  ): { columnsToUpdate: DbRecordColumnEntity[], columnsToCreate: DbRecordColumnEntity[] } {
    try {
      ...
      const columnsToCreate: DbRecordColumnEntity[] = [];
      const columnsToUpdate: DbRecordColumnEntity[] = [];

      if(dbRecordColumns.length < 1) {

        this.validateAllRequiredPropertiesExist(body, schema.columns, record.schemaTypeId);
