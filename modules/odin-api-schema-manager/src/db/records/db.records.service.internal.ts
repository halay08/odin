...
@Injectable()
export class DbRecordsServiceInternal extends DbRecordsColumnsService {
  ...
  public async _deleteDbRecordById(
    ...
    try {
      ...
      // get db record
      const record = await this._getDbRecordById(principal.organization, { recordId });
  ...
  public async _getManyDbRecordsByIds(
    ...
    try {
      ...
      const records = await this.dbRecordsRepository.getManyDbRecordsByOrganizationsAndIds(organization, recordIds);
  ...
  public async _updateOrCreateDbRecords(
    ...
    try {
      ...
      if(!!recordId && !schema.upsertOnCreate) {
        ...
        DbRecordsServiceInternal.throwRecordAlreadyExistsException(recordId, schema, body.title);
  ...
  public async _updateDbRecordById(
    ...
    try {
      ...
      if(!!body.properties) {
        await DbRecordsServiceInternal.validateCreateUpdateBody(body);
      }
      // Get the schema
      const schema = await this.schemasService.getSchemaByOrganizationAndEntityOrId(principal.organization, body);
  ...
  private async createDbRecordByPrincipal(
    ...
    try {
      ...
      if(!body.title){
        let titleParts: any=schema.columns.filter(elem => elem.isTitleColumn)
        if(titleParts.length){
          titleParts=titleParts.sort((a,b) => a.position - b.position)
          titleParts=titleParts.map(elem => body.properties[elem.name])
  ...
  private async validateRecordTitleConstraintsWithBody(body: DbRecordCreateUpdateDto, schema: SchemaEntity) {
    ...
    if(schema.hasTitle && schema.isTitleRequired && !body.title) {
      throw new ExceptionType(
        400,
        'a title is required',
        DbRecordsServiceInternal.createExceptionType('title', body.title, 'title is required'),
      );
    } else if(schema.hasTitle && schema.isTitleUnique && !body.title) {
      throw new ExceptionType(
        400,
        'a title is required',
        DbRecordsServiceInternal.createExceptionType('title', body.title, 'title is unique, please add one'),
      );
    }
  ...
  public async _getDbRecordById(
    ...
    const dbRecord = await this.dbRecordsRepository.getDbRecordByOrganizationAndId(organization, recordId);
