...
class FeatureDetailView extends React.Component<PropsType> {
  ...
  renderCustomButtons(record: DbRecordEntityTransform) {
    if(record) {
      switch (record.type) {
        case 'CLOSURE':
          return [
            <Link style={{ marginLeft: 8 }} to={`/ProjectModule/Feature/${record?.id}/configure-closure`}><Button
              type="primary">Configure</Button></Link>,
          ];
        default:
          return []
      }
        ...
        <Col xs={24} sm={24} md={24} lg={12}>
          <div className="record-detail-center-panel">
            ...
            <CardWithTabs
              ...
              extra={[
                <Link target="_blank"
                      to={`/PlanningModule/Map/${record?.type}/${getProperty(record, 'ExternalRef')}`}><Button
                  type="primary">View in Map</Button></Link>,
...
export default withRouter(connect(mapState, mapDispatch)(FeatureDetailView));
