...
class TaskDetailView extends React.Component<PropsType> {
  ...
  renderDynamicAssociations(record: DbRecordEntityTransform, relatedSchemas: SchemaEntity[]) {
    ...
    for(const schema of relatedSchemas) {
      ...
      if(schema.entityName === 'MILESTONE' && getProperty(record, 'Type') === 'EXCHANGE') {
        // @ts-ignore
        obj[schema.entityName] = <AssociationDataTable
          title="Milestones"
          record={record}
      ...
      } else if(schema.entityName === 'MILESTONE' && getProperty(record, 'Type') !== 'EXCHANGE') {
        // @ts-ignore
        obj[schema.entityName] = <AssociationDataTable
          title="Milestones"
          record={record}
