...
class AddPointModal extends React.Component<Props> {
  ...
  private handleFormSubmit(params: { event: string, results: any }) {
    ...
    if(mapReducer.map) {
      ...
      setTimeout(() => {
        ...
        const newZoomOut = Number(getZoom) + 0.000005;
        const newZoomIn = Number(getZoom) + 0.000010;
  ...
  render() {
    ...
    return (
      ...
      <>
        ...
        <Modal
          className="addPointModal"
          centered
          title="Add New Point"
          okButtonProps={{ style: { display: 'none' } }}
          visible={mapReducer.addPointModal && mapReducer.addEnabled}
          onCancel={() => {
            updateMap({ addPointModal: false })
          }}
        >
...
export default connect(mapState, mapDispatch)(AddPointModal);
