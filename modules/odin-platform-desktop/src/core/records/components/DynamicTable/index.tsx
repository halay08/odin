...
class DataTable extends React.Component<Props> {
  ...
  handleTableChange(pagination: any, localSort: any, colChange: any) {
    const { searchRecords, recordReducer, moduleName, entityName, schemaReducer, queryBuilderReducer } = this.props;
    const schema = getSchemaFromShortListByModuleAndEntity(schemaReducer.shortList, moduleName, entityName);
  ...
  render() {
    const { schemaReducer, recordReducer, recordTableReducer, maxHeight, queryBuilderReducer, schema } = this.props;
        ...
        <Row>
          <Col span={queryBuilderReducer.isVisible ? 19 : 24}>
            <Table
              ...
              size="small"
              // bordered
              loading={(recordReducer.isSearching && recordTableReducer.data.length < 1) || schemaReducer.isRequesting}
              rowSelection={{
                type: 'checkbox',
          ...
          <QueryBuilderCard schema={schema}/>
        </Row>
      </div>
    );
  }
...
const mapDispatch = (dispatch: any) => ({
  ...
  setQueryBuilder: (params: any) => dispatch(setQueryBuilderState(params)),
