...
import { Button, Card, Col, Collapse, Divider, Empty, Form, Input, Row, Select, Switch } from 'antd';
...
class EmbeddedForm extends React.Component<Props, State> {
  ...
  handleSubmit = async () => {
    const { createRecord, updateRecord, updateRecordAssociation, logVisit, notifyError, recordFormReducer, onSubmitEvent, closeForm } = this.props;
  ...
  private renderStageFilterOptions(schema: SchemaEntity | undefined) {
  ...
  render() {
    ...
    return (
      <>
        ...
        >
          <Row>
            <h2 title="5">Record Fields</h2>
            <Col span={24}>
              {this.renderTitleField(schema)}
              {this.renderSchemaTypeField(schema)}
              {this.renderRecordOwnerField(schema)}
              {this.renderStageFilterOptions(schema)}
              {/* {this.renderSelectInputForAssociations(schema)} */}
            </Col>
          </Row>
          <Divider/>
          <h2 title="5">Record Properties</h2>
          <Row>
            <Col span={12}>
          ...
          <Row>
            <Collapse ghost style={{ width: '100%' }}>
              <Panel header="Additional Properties" key="1">  
                <Row>
                  <Col span={12}>
...
export default connect(mapState, mapDispatch)(EmbeddedForm);
